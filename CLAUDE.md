...
**Env vars** (set in the Vercel project's dashboard, never committed): `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `GITHUB_TOKEN` (fine-grained PAT, Contents read/write, scoped to only this repo), `ALLOWED_ORIGIN` (CORS allow-list for `create-checkout-session`/`create-gcash-request`/`update-reservation-status`, e.g. `https://blueweyl.github.io` — must stay the bare origin, no path, to match the browser's `Origin` header), `SITE_BASE_URL` (optional override for the success/cancel redirect base; defaults to `https://blueweyl.github.io/hype-district-plaridel`, i.e. `ALLOWED_ORIGIN` **plus** the GitHub project-page path — these two intentionally differ in shape, don't try to derive one from the other). `PAYMONGO_SECRET_KEY`/`PAYMONGO_WEBHOOK_SECRET` are only needed if the dormant PayMongo path is ever reactivated. `RESEND_API_KEY` (+ optional `RESEND_FROM_EMAIL`) power the accept/reject emails below — if unset, `api/_lib/email.js#sendEmail()` just logs and skips, same silent-degrade pattern as the SukiDesk push.

**Receipts**: `api/_lib/email.js` wraps every outgoing email (booking confirmations, GCash approve/reject, in-person checkout receipts) in a shared `emailShell()` letterhead — "Hype District Barbers — Plaridel" branding, shop address/phone footer — so nothing goes out looking like a bare transactional message. Card (Stripe) payments email a receipt immediately from `api/stripe-webhook.js` right after `confirmReservation()` succeeds (there's no separate staff-approval step for card, unlike GCash). GCash's receipt is folded into the existing "approve" email from `api/update-reservation-status.js` (see below) rather than being a separate send. Both online receipts show the combined service name + total price as a single line (not itemized per-service — see "Multi-service bookings" above for why individual prices aren't threaded that far).

**Staff accept/reject of GCash payments** (`api/update-reservation-status.js`, driven from `app.html`'s Reservations tab, see below): unlike the customer-facing endpoints above, this one is gated by `SUKIDESK_SECRET` (the same shared secret already used for the Sheet bridge — sent as `token` in the request body, read from the app's own Settings-tab value) since it lets a caller flip a reservation's status and free its slot; skip the check only if `SUKIDESK_SECRET` isn't configured at all, mirroring `Code.gs`'s own fallback. It calls `api/_lib/reservation.js#resolvePendingReservation()` — the one place a reservation file is *updated* (not create-only-written) — to set `status: 'confirmed'` or `'payment_rejected'` on the public repo JSON, releasing the slot back to `availability.json` on reject, then emails the customer via `api/_lib/email.js#sendEmail()`.

**Multi-service bookings**: `reserve.html`'s service picker is multi-select (checkboxes, not radios) — a customer can pick several services (e.g. haircut + beard trim) for one booking. By design they all still share a single fixed-length slot (`hours.slotMinutes`) rather than each consuming its own consecutive slot — `api/_lib/booking-validate.js#validateBooking()` takes `serviceIds` (array) and returns `totalPrice` (sum) and `combinedName` (joined with `" + "`). Downstream, a reservation's `serviceId` field becomes a comma-joined string of ids and `serviceName` the combined display name — same shape as a single-service booking, just occasionally holding multiple values, so nothing downstream (SukiDesk, Decap CMS, `js/reservations.js`) needed schema changes. Stripe/PayMongo checkout sessions get one line item per service, so the hosted payment page itemizes them individually even though they're one combined charge.
...
A separate internal tool, not part of the public marketing site and not linked from its nav/footer. It's a queue/checkout/CRM app for front-desk staff: PIN login per staff member (`screen-login`), then tabbed views (Queue, Clients, Checkout, Reservations, Dashboard, and owner-only Staff/Settings) in `screen-app`.

- **In-person checkout receipts**: the Checkout tab has an optional "Email (for receipt)" field (`checkout-client-email` in `app.html`) — if filled in, `js/app.js#sendCheckoutReceipt()` POSTs the itemized cart to `api/send-checkout-receipt.js` (same `SUKIDESK_SECRET`-gated trust model as `update-reservation-status.js`), which emails a proper itemized receipt via `api/_lib/email.js#checkoutReceiptEmail()`. It's best-effort and doesn't block/alert on failure the way Reservations tab actions do, since the transaction itself is already logged locally either way. The email (if given) is also saved onto the client's record (`email` field, added to `SCHEMAS.clients` in `Code.gs` — update that if this field's name ever changes) so it can be reused on their next visit.

- **Storage model**: `js/app.js` keeps the whole DB (`staff`, `clients`, `bookings`, `transactions`, `services`) in `localStorage` (`sukidesk_db_v1`) as the always-on, offline-tolerant source of truth. If a Google Sheets Web App URL + secret token are configured in the Settings tab, it additionally syncs the entire DB: pulls on login, pushes ~1.5s after any change (`pushToCloud`/`pullFromCloud`). The **Reservations** tab additionally auto-pulls every 30s while it's the active tab (`startReservationsPolling()`/`stopReservationsPolling()` in `js/app.js`, started/stopped from `goTab()`) so new online bookings and other devices' Approve/Reject actions show up without a manual "Pull from Cloud Now" — this is the one view that self-refreshes; every other tab still requires a manual pull or fresh login.
...
- **GCash payment-proof screenshots**: for `payment_pending` GCash manual-transfer bookings, `booking.proofImageBase64`/`proofImageMimeType` ride along in that same `addBooking` payload; `Code.gs` decodes and saves the image to a Drive folder named **"SukiDesk Payment Proofs"** (shared view-only-with-link) and stores the resulting URL in the row's `proof_url` column instead of the raw image data — the base64 itself is never written to a sheet cell. `gcash_reference` (optional, customer-entered) is stored alongside it. This is the only path in the app where a payment still needs manual staff verification: the **Reservations** tab shows a "View" link to the screenshot and, for any row with `status: 'payment_pending'`, **Approve**/**Reject** buttons (`resolvePendingPayment()` in `js/app.js`). Either one updates the booking's status in the local DB immediately (any logged-in staff member can do this, no owner-only gate) and calls `api/update-reservation-status.js` on the Vercel API to update the public repo's copy of the reservation, release the slot on reject, and email the customer their confirmation/rejection via Resend — see "Online reservations" above for the server side of this.
