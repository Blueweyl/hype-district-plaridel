...
- **Online reservations bridge**: `api/stripe-webhook.js` optionally pushes each paid online booking (full `client_name`/`client_contact`/`client_email` — never written to the public repo) directly into the Sheet's `Bookings` tab via a dedicated `addBooking` action in `Code.gs` (appends one row, unlike `saveAll` which overwrites a whole tab). Gated by two Vercel env vars, `SUKIDESK_WEBAPP_URL` + `SUKIDESK_SECRET` (must match the Apps Script's `SUKIDESK_SECRET` script property) — if unset, the webhook silently skips this and the reservation write to the public repo (see below) proceeds unaffected. Staff see these under the app's **Reservations** tab (`source: 'online'` bookings), which is the only place full customer contact info for online bookings lives outside Stripe itself.
